...
function Set-Env {
  ...
  $env:CFLAGS   = "-march=$march -mtune=generic -pipe -fstack-protector-strong -g -ggdb" # remove -O3
  $env:CXXFLAGS = "-D_FORTIFY_SOURCE=2 -march=$march -mtune=generic -pipe" # remove -O3
  $env:CPPFLAGS = "-D_FORTIFY_SOURCE=2 -D__USE_MINGW_ANSI_STDIO=1 -DFD_SETSIZE=2048"
  $env:LDFLAGS  = "-l:libssp.a -l:libz.a -pipe -fstack-protector-strong" # remove -s so it does add symbols
}
...
Run "sh -c `"../ruby/configure optflags=-O0 --disable-install-doc --prefix=$d_install $config_args`"" {
  sh -c "../ruby/configure optflags=-O0 --disable-install-doc --prefix=$d_install $config_args"
}
Time-Log "configure"
